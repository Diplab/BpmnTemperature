...
#define         CALIBARAION_SAMPLE_TIMES     (1)    //define how many samples you are going to take in the calibration phase
...
{ 
  return (pow(10,( (((log(rs_ro_ratio)/log(10))-pcurve[1])/pcurve[2]) + pcurve[0])));
}
...
double readTemp(){
    FILE *fptr;
    char buffer[100];
    fptr = fopen("/sys/bus/w1/devices/28-0000051f2e41/w1_slave","r");
    if(fptr==NULL){
        printf("error\n");
        return -1;   
    }
    while( ! feof (fptr) ){
        ...
        char *temp=strstr(buffer, "t=");
        if(temp != NULL){
            fclose(fptr);
            double out = atof(temp+2);
            return out/1000;
        }
    }
    fclose(fptr);
